...
async def message_handler(msg: Message):
    async with aiohttp.ClientSession() as client_session:
        ...
        for game in game_price_request:
            game_choice_keyboard.row(types.InlineKeyboardButton(
        ...
        async with (aiohttp.ClientSession() as client_session):
            ...
            for market in game_data['markets']:
                if market['price'] is not None:
                    prices_user_message = text.games_prices_message.format(
                        game_name=game_data[
                            'name']) + f"🕹️ {market['name']}: {market['price'] / 100}{market['currency']}\n"
                else:
                    prices_user_message = text.games_prices_message.format(
